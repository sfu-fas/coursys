...
def committee_members(emplids):
    ...
    db = SIMSConn()
    db.execute("""
        SELECT distinct 'CommitteeMembership', ST.EMPLID, ST.COMMITTEE_ID, SUBSTRING(COM.DESCR, 11, 5) AS ACAD_PROG, COM.EFFDT, COM.COMMITTEE_TYPE, MEM.EMPLID, MEM.COMMITTEE_ROLE
        FROM
