...
def committee_members(emplids):
    ...
    db.execute("""
        WITH CM AS (
            SELECT SC.EMPLID AS S_EMPLID, SC.ACAD_CAREER, SC.STDNT_CAR_NBR, SC.COMMITTEE_ID, COM.EFFDT, COM.COMMITTEE_TYPE, MEM.EMPLID, MEM.COMMITTEE_ROLE,
            ROW_NUMBER() OVER (PARTITION BY MEM.EMPLID, MEM.COMMITTEE_ROLE ORDER BY COM.EFFDT DESC) AS RN,
        ...
        SELECT 'CommitteeMembership', CM.S_EMPLID AS EMPLID, CM.ACAD_CAREER, CM.STDNT_CAR_NBR, CM.COMMITTEE_ID, CM.EFFDT, CM.COMMITTEE_TYPE, CM.EMPLID, CM.COMMITTEE_ROLE, CM.MAX_EFFDT
        FROM CM
        WHERE RN = 1
        ORDER BY EFFDT""",
        (emplids,))
