    ...
    db.execute("""
        ...
        strm_Data AS 
            (SELECT *, RANK () OVER (PARTITION BY BUSINESS_UNIT ORDER BY STRM DESC) AS date_Rank 
                ...
                WHERE EFF_STATUS = 'A')
        SELECT 'ScholarshipDisbursement', EMPLID, AID_YEAR, ITEM_TYPE, ACAD_CAREER, DISBURSEMENT_ID, STRM, DESCR, DISBURSED_BALANCE, ACAD_PROG FROM (
            SELECT SA.EMPLID, SA.AID_YEAR, SA.ITEM_TYPE, SA.ACAD_CAREER, SA.DISBURSEMENT_ID, SA.STRM, IT.DESCR, SA.DISBURSED_BALANCE, org_Status.ACAD_PROG,
                ...
                JOIN (SELECT DATA1.EMPLID, DATA1.ACAD_CAREER, DATA1.STDNT_CAR_NBR, DATA1.ACAD_PROG, DATA1.ADMIT_TERM, DATA1.COMPLETION_TERM, SD.STRM AS LATEST_TERM, DATA1.translated_Status AS END_STAT 
                        FROM (	
                            SELECT *, RANK () OVER (PARTITION BY EMPLID, ACAD_CAREER, STDNT_CAR_NBR ORDER BY EFFDT DESC, EFFSEQ DESC) AS date_Rank
                                FROM status_Org DATA1
                                WHERE DATA1.translated_Status NOT IN ('None') AND DATA1.ACAD_PROG IN ('CPPHD', 'CPPZU', 'CPMSC', 'CPMZU', 'CPMBD', 'CPGQL', 'CPMCW', 'CPGND', 'PMSCS', 'CPMC', 'CPMVC', 'CPPMS', 'MSEPH', 'MSEMS', 'MESMS', 'MSEGX', 'MECHVRS', 'ESMEN', 'ESMAS', 'ESPHD', 'PSGEX', 'PSGND', 'PSGQL', 'PSMAC', 'PSMAP', 'PSPHC', 'PSPHP', 'SEMAS', 'SEPHD')
                                ) DATA1
                            JOIN (SELECT sd1.STRM, sd1.TERM_END_DT AS TERM_END, sd2.TERM_END_DT AS PREV_TERM_END
                                    FROM strm_Data sd1
