...
def _get_grads_missing_supervisors(units):
    grads = GradStudent.objects.filter(program__requires_supervisor=True, current_status__in=STATUS_ACTIVE, program__unit__in=units).order_by("start_semester", "person__last_name", "person__first_name", "program__unit", "program__label")
