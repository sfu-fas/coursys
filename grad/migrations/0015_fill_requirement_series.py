...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for req in orm['grad.GradRequirement'].objects.all():
            others = orm['grad.GradRequirement'].objects \
                    .filter(description=req.description,
                            program__unit=req.program.unit) \
                    .exclude(series=None)
            if others:
                ...
                used = set(r.series for r in orm['grad.GradRequirement'].objects.all())
                used.remove(None)
                try:
                    ser = max(used) + 1
                except ValueError:
