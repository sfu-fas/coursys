...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Copy SIN from GradStudent to Person object, where it should live."
        for gs in orm['grad.GradStudent'].objects.all():
            if 'sin' in gs.config and gs.config['sin'] not in [0, 'N/A']:
                sin = unicode(gs.config['sin'])
                oldsin = gs.person.config.get('sin', None)
                if not oldsin:
                    p = gs.person
                    p.config['sin'] = sin
                    p.save()
