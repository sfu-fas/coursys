...
forms_patterns = [
    ...
    url(r'^admin/bulk_close/pending', onlineforms_views.bulk_close, kwargs={'status': 'pending'}, name='bulk_close_pending'),
    url(r'^admin/bulk_close/waiting', onlineforms_views.bulk_close, kwargs={'status': 'waiting'}, name='bulk_close_waiting'),
    ...
    url(r'^view/' + FORM_SLUG + '/' + FORMSUBMIT_SLUG + r'/(?P<action>\w+)/(?P<file_id>\d+)/(?P<secret>\w+)$', onlineforms_views.file_field_download_unauth, name='file_field_download_unauth'),
