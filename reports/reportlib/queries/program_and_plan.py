...
class ActivePlanQueryWithReqTerm(DB2_Query):
    ...
    INNER JOIN
        PS_STDNT_CAR_TERM CAR_TERM 
        ON
        APLAN.EMPLID = CAR_TERM.EMPLID AND
        APLAN.ACAD_CAREER = CAR_TERM.ACAD_CAREER AND
        APLAN.STDNT_CAR_NBR = CAR_TERM.STDNT_CAR_NBR
    INNER JOIN 
        PS_ACAD_PROG PROG
        ON
        APLAN.EMPLID = PROG.EMPLID AND
        APLAN.ACAD_CAREER = PROG.ACAD_CAREER AND
        APLAN.STDNT_CAR_NBR = PROG.STDNT_CAR_NBR AND
        APLAN.EFFDT = PROG.EFFDT AND
        APLAN.EFFSEQ = PROG.EFFSEQ 
    WHERE 
        APLAN.ACAD_PLAN IN $plans
        AND PROG.PROG_STATUS = 'AC' 
        AND CAR_TERM.WITHDRAW_CODE = 'NWD'
        AND CAR_TERM.STRM < $registration_semester
        AND CAR_TERM.STRM >= $third_prior_semester
        AND APLAN.EFFDT = (
            ...
            WHERE APLAN.EMPLID = TEMP_PLAN.EMPLID AND
                  APLAN.ACAD_CAREER = TEMP_PLAN.ACAD_CAREER AND 
                  APLAN.ACAD_PLAN = TEMP_PLAN.ACAD_PLAN AND
                  TEMP_PLAN.EFFDT < $effective_date)
        AND APLAN.EFFSEQ = (
            SELECT MAX(TEMP_PLAN_2.EFFSEQ)
            FROM PS_ACAD_PLAN TEMP_PLAN_2
            WHERE APLAN.EMPLID = TEMP_PLAN_2.EMPLID AND
                  APLAN.ACAD_CAREER = TEMP_PLAN_2.ACAD_CAREER AND 
                  APLAN.ACAD_PLAN = TEMP_PLAN_2.ACAD_PLAN AND 
                  APLAN.EFFDT = TEMP_PLAN_2.EFFDT)
        """)
    default_arguments = {
        'plans': ['CMPTMAJ'],
        'effective_date': datetime.datetime.now(), 
...
class SubplanQuery(DB2_Query):
    ...
    INNER JOIN 
        ...
        APLAN.EMPLID = PROG.EMPLID AND
        APLAN.ACAD_CAREER = PROG.ACAD_CAREER AND
        APLAN.STDNT_CAR_NBR = PROG.STDNT_CAR_NBR
    WHERE 
        PROG.PROG_STATUS = 'AC' 
        AND CAR_TERM.WITHDRAW_CODE = 'NWD'
        AND CAR_TERM.STRM < $registration_semester
        ...
        AND APLAN.EFFDT = (
            ...
            WHERE APLAN.EMPLID = TEMP_PLAN.EMPLID AND
                  APLAN.ACAD_CAREER = TEMP_PLAN.ACAD_CAREER AND 
                  APLAN.ACAD_PLAN = TEMP_PLAN.ACAD_PLAN AND
                  APLAN.ACAD_SUB_PLAN = TEMP_PLAN.ACAD_SUB_PLAN AND
                  TEMP_PLAN.EFFDT < $effective_date)
        AND APLAN.EFFSEQ = (
            SELECT MAX(TEMP_PLAN_2.EFFSEQ)
            ...
            WHERE APLAN.EMPLID = TEMP_PLAN_2.EMPLID AND
                  APLAN.ACAD_CAREER = TEMP_PLAN_2.ACAD_CAREER AND 
                  APLAN.ACAD_PLAN = TEMP_PLAN_2.ACAD_PLAN AND 
                  APLAN.ACAD_SUB_PLAN = TEMP_PLAN_2.ACAD_SUB_PLAN AND
                  APLAN.EFFDT = TEMP_PLAN_2.EFFDT AND
