...
class MSECourseHistoryReport(Report):
    title = "MSE Course History"
    description = "This report summarizes course information for MSE, including enrollment, instructors, etc"

    def run(self):
        sems = Semester.objects.filter(name__gte='1001', name__lte=Semester.next_starting().name)
        u = Unit.objects.get(label='MSE')
