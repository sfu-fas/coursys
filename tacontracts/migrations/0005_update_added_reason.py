...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        Member = orm['coredata.Member']
        for crs in orm.TACourse.objects.all():
            # for each course being TAed as a result of a contract here...
            p = crs.contract.person
            o = crs.course
            members = Member.objects.filter(person=p, offering=o, role='TA', added_reason='CTA')
            if members:
                # if there is a Member, change its added_reason
                m = members[0]
                m.added_reason = 'TAC'
                m.save()
    ...
    def backwards(self, orm):
        Member = orm['coredata.Member']
        for m in Member.objects.filter(role='TA', added_reason='TAC'):
            m.added_reason = 'CTA'
            m.save()
    ...
    models = {
        ...
        u'tacontracts.tacategory': {
            ...
            'config': ('courselib.json_fields.JSONField', [], {'default': '{}'}),
            'created': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 10, 1, 0, 0)'}),
            'hidden': ('django.db.models.fields.BooleanField', [], {'default': 'False'}),
