...
The commands in `deploying/01-bootstrap.sh` are intended to be run manually to get the code in place.

After that, the scripts in `deploying/` should be able to set up the basics on the server. Have a look
at `deploying/config.sh` first, Contents likely something like:
```bash
...
Review and run scripts 02-05 in `deploying/`. Have a look around and make sure things are as expected.
