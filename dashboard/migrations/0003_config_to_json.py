...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # make sure all UserConfig.value's are JSON so we can switch to JSONField
        for uc in orm.UserConfig.objects.filter(key='feed-token'):
            token = uc.value
            uc.value = json.dumps({'token': token})
            uc.save()
