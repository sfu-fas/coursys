...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding field 'Unit.config'
        db.add_column('coredata_unit', 'config', self.gf('jsonfield.JSONField')(default={}), keep_default=False)
    ...
    def backwards(self, orm):
        ...
        # Deleting field 'Unit.config'
        db.delete_column('coredata_unit', 'config')

        # Adding field 'Role.department'
        db.add_column('coredata_role', 'department', self.gf('django.db.models.fields.CharField')(default='????', max_length=4), keep_default=False)

        # put department label into Role.department
        for role in orm.Role.objects.all():
            # 'UNIV' is the new global permission
            dept = role.unit.label
            if dept == 'UNIV':
                dept = '!!!!'
