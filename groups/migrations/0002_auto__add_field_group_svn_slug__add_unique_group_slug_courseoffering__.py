...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding field 'Group.svn_slug'
        db.add_column('groups_group', 'svn_slug', self.gf('autoslug.fields.AutoSlugField')(default=None, max_length=17, unique=False, unique_with=('courseoffering',), db_index=True, null=True), keep_default=False)

        # Adding unique constraint on 'Group', fields ['slug', 'courseoffering']
        db.create_unique('groups_group', ['slug', 'courseoffering_id'])

        # Adding unique constraint on 'Group', fields ['courseoffering', 'svn_slug']
        db.create_unique('groups_group', ['courseoffering_id', 'svn_slug'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'Group', fields ['courseoffering', 'svn_slug']
        db.delete_unique('groups_group', ['courseoffering_id', 'svn_slug'])

        # Removing unique constraint on 'Group', fields ['slug', 'courseoffering']
        db.delete_unique('groups_group', ['slug', 'courseoffering_id'])

        # Deleting field 'Group.svn_slug'
        db.delete_column('groups_group', 'svn_slug')
    ...
    models = {
        ...
        'groups.group': {
            ...
            'svn_slug': ('autoslug.fields.AutoSlugField', [], {'max_length': '17', 'unique': 'False', 'unique_with': "('courseoffering',)", 'db_index': 'True'})
        },
        'groups.groupmember': {
