...
The library to do this is Ruby-only, so there's some complexity. Two methods are implemented here:
 1. an RPC call using RabbitMQ to a docker container. The container is defined in the ruby-markup directory of this repo
    and managed by docker-compose). The response is ultimately generated by markdown2html-server.rb.
 2. starting a Ruby subprocess and capturing its output.
 3. the Python markdown library, with its different results
