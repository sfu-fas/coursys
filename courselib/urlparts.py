COURSE_SLUG_RE = r'(?:\d{4}|\d{4}[a-z]{2})-[a-z]{2,5}-\w{3,4}-[a-z0-9]{2,4}'
SLUG_RE = r'[\w\-\.]+'
...
COURSE_ACTIVITY_SLUG = COURSE_SLUG + r'/\+' + ACTIVITY_SLUG
COURSE_ACTIVITY_SLUG_OLD = COURSE_SLUG + '/' + ACTIVITY_SLUG

USERID_SLUG = r'(?P<userid>[\w\-]+)'
...
CASE_SLUG = r'(?P<case_slug>[\w\-]+)'
DGROUP_SLUG = r'(?P<group_slug>[\w\-]+)'

SEMESTER = r'(?P<semester>\d{4})'
PAGE_LABEL = r'(?P<page_label>[\w\-_\.]+)'
